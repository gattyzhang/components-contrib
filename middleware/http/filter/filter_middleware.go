...
type oFilterMiddlewareMetadata struct {
	...
	Filter_check_err_body    string `json:"filter_check_err_body"`
	Filter_gray_run_err_body string `json:"filter_gray_run_err_body"`
	Tmp_parms                []string
	Tmp_skip_uri             []string
...
func NewOFilterMiddleware(logger logger.Logger) *Middleware {
...
func (m *Middleware) grayRunHandler(ctx *fasthttp.RequestCtx, val_filter_err_url []byte) error {
	...
	ctx.Request.Header.VisitAll(func(k []byte, v []byte) {
		m.logger.Debug("----1.5.1 copy reqeust ", string(k), string(v))
	...
	m.logger.Debug("gatty---3.4-- GrayRun ", string(respGray.Body()))

	//copy back the returns
	ctx.Response.SetBody(respGray.Body())
	respGray.Header.VisitAll(func(k []byte, v []byte) {
		m.logger.Debug("---- 1.5.2 copy back the response ", string(k), string(v))
...
func (m *Middleware) GetHandler(metadata middleware.Metadata) (func(h fasthttp.RequestHandler) fasthttp.RequestHandler, error) {
	m.logger.Debug("gatty---1-- middleware.Metadata: ", metadata)
	meta, err := m.getNativeMetadata(metadata)
	if err != nil {
		return nil, err
	}
	m.logger.Debug("gatty---2-- after formatting, meta: ", meta)

	return func(h fasthttp.RequestHandler) fasthttp.RequestHandler {

		return func(ctx *fasthttp.RequestCtx) {
			m.logger.Debug("---gatty.3.6--- original request body:", string(ctx.Request.Body()))
			// 0. 如果是跳过的URI，就直接往后处理。
			uri := ctx.RequestURI()
			...
			ctx.Request.Header.VisitAll(func(k []byte, v []byte) {
				m.logger.Debug("---- header>", string(k), string(v))
				if bytes.Equal(header_key_cookie_key, k) {
					if meta.Tmp_cookie_flag == true {
						//1. skip the cookie header
						m.logger.Debug("---- pass through the cookie item:"+string(k), ",", string(v))
					}
			...
			if err != nil {
				m.logger.Error("生成filter请求的参数时异常-json。", err)
				return
			}
			...
			if err := fasthttp.Do(req, resp); err != nil {
				m.logger.Error("failed to send request to filter micro service, return 401 of status code.", err.Error())
				ctx.Response.SetStatusCode(401)
				ctx.Response.SetBody([]byte(meta.Filter_check_err_body))
				return
			}
			//1.4 处理filter的结果
			val_filter_err_url := resp.Header.Peek(filter_err_url)
			val_flag_forward := string(resp.Header.Peek(header_key_flag_forward))
			m.logger.Debug("---- 1.4  filter_err_url:", string(val_filter_err_url), ",flag-forward:", val_flag_forward)

			if val_flag_forward == header_key_flag_forward_Redirect {
				// redirect to new location now.
				m.logger.Debug("gatty---3.3-- REDIRECT(no filter's outputs attached.) ", string(resp.Body()))
				...
				// normal
				m.writeBackHeaders(meta, ctx, resp)
				m.logger.Debug("gatty---3.2-- NORMAL ", string(resp.Body()))
